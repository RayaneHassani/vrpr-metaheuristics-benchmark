"""Instance generator for the VRPR problem.

Builds a random road network as a NetworkX graph. Node 0 is the depot.
Edges carry a ``cout`` (travel cost), a ``close`` flag (impassable road) and
a ``surcout`` flag (surcharged road).
"""
...
def random_graph(vertice_count=None, k_voisins=4, taux_fermeture=0.1,
                 taux_surcout=0.08, facteur_surcout=2.0, complet=False):
...
def draw_instance(G, ax=None):
    """Plot a VRPR instance: depot in red, closed edges dashed red, surcharged orange."""
    import matplotlib.pyplot as plt

    if ax is None:
        plt.figure(figsize=(12, 8))

    couleurs = ['red' if node == 0 else 'white' for node in G.nodes()]
    ...
    nx.draw_networkx_edges(G, pos, edgelist=aretes_normales, edge_color='black', width=1, ax=ax)
    nx.draw_networkx_edges(G, pos, edgelist=aretes_fermees, edge_color='red', width=1.5, style='dashed', ax=ax)
    nx.draw_networkx_edges(G, pos, edgelist=aretes_surcout, edge_color='orange', width=2, ax=ax)
    nx.draw_networkx_nodes(G, pos, node_color=couleurs, node_size=300, edgecolors='black', ax=ax)
    nx.draw_networkx_labels(G, pos, font_size=9, ax=ax)

    labels_cout = {(i, j): f"{G[i][j]['cout']:.0f}" for (i, j) in G.edges() if not G[i][j]['close']}
    nx.draw_networkx_edge_labels(G, pos, edge_labels=labels_cout, font_size=7, ax=ax)
    if ax is None:
        plt.show()
